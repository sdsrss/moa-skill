...
<p>
<img alt="status" src="https://img.shields.io/badge/status-v1.6.0-brightgreen"> <img alt="tests" src="https://img.shields.io/badge/tests-170%20passing-brightgreen"> <img alt="python" src="https://img.shields.io/badge/python-3.9%2B-blue"> <img alt="license" src="https://img.shields.io/badge/license-MIT-green">
</p>
...
一个模型评审自己的产出，只有一套盲点。MoA Skill 召集 **至多 4 个独立异构模型**（横跨 OpenAI / Anthropic / Google / Moonshot 家族），生成阶段互不可见对方原始产出，再由**持完整上下文的当前 agent 仲裁**，受一套反群体思维硬规则约束。你得到的是**真正独立**的第二、第三、第四意见，分歧被保留而非被磨平。

> **关于默认阵容（v1.5.0）：** 发货的 `config.example.yaml` 现在是**四家族**——OpenAI(gpt5.6-sol 走 auggie,A 席) + Anthropic(Opus 子代理,B 席) + Google(gemini-3.1-pro 走 auggie,C 席) + **Moonshot(kimi-k2.7 走 auggie,D 席)**。v1.4.0 的默认在 D 席放第二个 Anthropic 模型(角色分化的 Self-MoA);v1.5.0 审核指出:那时 B 席、D 席**加上你的 Claude 仲裁人**全是 Anthropic(5 个判断主体里 3 个同家族),"全员一致"的独立性弱于表面。D 席现在默认走一个真正的第四家族(走 auggie,同一账号,不需额外 key)。想回到更省的 Self-MoA 席,把 D 席换回 Opus 子代理即可——具体那段已在 [`config.example.yaml`](skills/moa/assets/config.example.yaml) 里注释好。家族数是配置选择，并非硬编码为四。
...
pip install pyyaml
# 可选 CH2（本地 CLI 通道）：装 auggie（`auggie login`；检测到即优先——一个账号覆盖
#   GPT/Gemini/Claude/Kimi/GLM）和/或 codex CLI（0.144+）
```
...
| **CH2 CLI** | `cli_kind: auggie`（`auggie --print`、JSON 信封、空 workspace 沙箱；**检测到即优先**,`cli_kind: codex` 可退出）或 `codex exec`（`-s read-only`、prompt 走 stdin） | auggie：上游价 +40%（Augment 套餐）；codex：订阅 | `moa.py` |
...
- **`full`**（手动 `/moa <材料>`）——固定 4 席顶配 + 仲裁（默认四家族：OpenAI / Anthropic / Google / Moonshot）。
...
Token 约为单模型的数倍。实测（2 个计费席 + 1 精炼轮，用的是便宜的非推理模型，见 [`COST-NOTE.md`](moa-reports/cost-m4/COST-NOTE.md)）= **4.79× 基线**。**发货默认（v1.5.0）现在计费三席——A、C、D 走 auggie**（上游 API 价 +40%，从 Augment 套餐余额扣；只有 B 席仍为订阅子代理）。这比 v1.4.0 多一个计费席:D 席从免费的 Anthropic Self-MoA 子代理换成 Moonshot/kimi-k2.7,换来真正的第四家族——故默认真实成本高于 4.79×（那是 2 计费席配置下的实测值）。想回到 1.5.0 之前更省的组合,把 D 席换回 Opus 子代理（`config.example.yaml` 里已注释）；想彻底去掉 auggie 计费,A 席设 `cli_kind: codex`、C 席改 `channel: api`。**全 CH3 四席 + 精炼轮 ≈ 9.6×**（超 7× 目标）。永远先 `dry-run` 给用户看成本估算再正式跑。
...
| 现象 | 原因 | 处理 |
|---|---|---|
| `FAIL[empty]` / `FAIL[truncated]` | 推理型模型把 `max_tokens` 预算耗在推理上，正文空/被截断 | v1.4.0 重试时自动倍增预算（封顶 16000）；仍失败再调大 `max_tokens_member` 或该席改用非推理模型 |
...
| 顶配推理模型（gpt-5 / gemini-pro）跑很久后 `FAIL[empty]` | 推理吃光 `max_tokens`，正文空壳（**假阴性**，模型其实可用） | v1.4.0 已自动倍增预算重试；decide 长输出仍建议 `max_tokens_member` ≥ 8000 起步，或把该席挪到 auggie（预算由它自管）|
...
| `auggie not found on PATH` | CH2 席 `cli_kind: auggie` 但未装 auggie | 装 auggie（`npm i -g @augmentcode/auggie` + `auggie login`），或该席改 `cli_kind: codex` / `channel: api` |
| auggie 席并发时挂到超时 | Augment 503 + 内部重试（实测无上限时 >7 分钟） | 必设 `timeout_seconds`（硬兜底）；内置 `--retry-timeout` 已限内部重试 |
...
CI 在 push/PR 上跨 Python 3.9 与 3.12 跑测试套件 + `leak-check`，并在发版 tag 上校验版本/徽章一致性（`.github/workflows/ci.yml`）。

**平台：** 已在 Linux 与 macOS 实测。Windows 未验证——`subprocess`/路径处理大概率可用，但 auggie/codex CLI 的 Windows 行为未跑过。
