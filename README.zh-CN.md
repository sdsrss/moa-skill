...
> **关于默认阵容（v1.4.0）：** 发货的 `config.example.yaml` 实际是三家族——OpenAI(gpt5.6-sol 走 auggie) + Anthropic(Opus 子代理) + Google(gemini-3.1-pro 走 auggie)——外加第四席**角色分化的 Self-MoA**(第二个 Anthropic 模型扮反方)。第四个*家族*现在只差一段注释：把 D 席换成 auggie 的 `kimi-k2.7`(Moonshot 家族,同一账号,不需额外 key)。旧的 xAI/Grok 方案需有 x-ai 供给的 OpenRouter key(多数 key 为 list-only 404)，备忘见 [`config.example.yaml`](skills/moa/assets/config.example.yaml)。家族数是配置选择，并非硬编码为四。
...
Token 约为单模型的数倍。实测（2 个计费席 + 1 精炼轮，用的是便宜的非推理模型，见 [`COST-NOTE.md`](moa-reports/cost-m4/COST-NOTE.md)）= **4.79× 基线**。**发货默认（v1.4.0）A 与 C 席走 auggie 计费**（上游 API 价 +40%，从 Augment 套餐余额扣；B 与 D 仍为订阅子代理）——这是无头 codex 超时、OpenRouter 推理模型空壳之后**用成本换稳定**的有意取舍；A 席设 `cli_kind: codex`、C 席改 `channel: api` 可回到 1.4.0 之前更省的组合。**全 CH3 四席 + 精炼轮 ≈ 9.6×**（超 7× 目标）。永远先 `dry-run` 给用户看成本估算再正式跑。
...
```bash
python -m pytest skills/moa/tests/ -q      # 162 项(行为测试 + 文档一致性校验)，无网络
