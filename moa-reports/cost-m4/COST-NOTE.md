...
- **按用户实付 token 计**：默认 config 四席里 A（codex CLI）、B（子代理 Claude）走**订阅通道不计费**，只有 C/D 两席 CH3 计费。故用户实付倍数 ≈ 实测 2 席值 **4.79× ≤ 7× ✓**（含 1 精炼轮）。
  > **更新（v1.1.5，2026-07-12）**：本记录测于 2026-07-11 的旧默认 config。此后 **D 席也改为免费 Opus 子代理**，发货默认**仅 C 席 CH3 计费**（1 个计费席）。结论方向不变且更省——用户实付倍数只会低于此处 4.79×（该值系 2 计费席 + 便宜非推理模型实测，仅作量级参考）。
